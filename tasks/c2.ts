...
task("C2", "crit 2")
  .setAction(async (taskArgs, { ethers }) => {
    ...
    const priceOracleHach = await ethers.deployContract("PriceOracleHach", [], accountOwnerContracts);
    await priceOracleHach.waitForDeployment();

    // 2. set hack PriceOracle
    await positionManager.connect(accountOwnerContracts).setPriceOracle(priceOracleHach.target);
